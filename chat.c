...
int main(int argc, char *argv[]) {
    ...
    int clientid;
    /*Add in*/
    char name_array[MAX_NAME_SIZE+1];
    int first = 1;
    ...
    if(argc==2 || argc==4)
    {
        ...
        while (1)
        {
            ...
            for(fd=0;fd<FD_SETSIZE;fd++)
            {
                ...
                {
                    if (first == 1 && fd == sockfd)
                    {
                        first = 0;
                        printf("Please Enter your Nickname to Server (MAX up to 12) : ");
                        fgets(kb_msg, MAX_NAME_SIZE +1, stdin);
                        sprintf(name_array, "%s", kb_msg);
                        for(i=0;i<=MAX_NAME_SIZE+1;i++)
                        {
                            if (name_array[i] == 10)
                            name_array[i] = 0;
                        }
                        write(sockfd, name_array, strlen(msg));
                    }
                    if(fd==sockfd)
                    ...
                    {   /*process keyboard activiy*/
                        ...
                        else
                        {
                            ...
                            msg[result]='\0';
                            strcat(kb_msg,msg+1);*/
                            sprintf(msg, "M<%s> : %s", name_array, kb_msg);
                            write(sockfd, msg, strlen(msg));
                        }
                    }          
                }
                ...
                {              
                    ...
                    {   /* Accept a new connection request */
                        ...
                        if (num_clients < MAX_CLIENTS)
                        {
                            ...
                            sprintf(msg,"MHello Client! Your UID is : %2d\n",client_sockfd);
